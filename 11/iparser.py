...
class Parser:
    ...
    def gType(self):
        ...
        # branch4 className
        chil_node = self.gClassName().children[0] # or的最后一个条件必须成功解析，否则就报错
        return chil_node
    ...
    def gDoStatement_safe(self):
        ...
        try:
            ...
            doNode.add(self.gSubRoutineCall())
    ...
    def gTerm(self):
        ...
        # subroutineCall (不止LL1，XXX.XXX), 改变解析位置只是个取巧的方法（贪心解析）
        checkpointB = self.ctid
        try:
            chil_node = self.gSubRoutineCall()
        except SyntaxError:
            self.ctid = checkpointB
        else:
            node.add(chil_node)
    ...
    def gSubRoutineCall(self):
        ...
        try:
            ...
            self.ctid = checkpointB
            raise SyntaxError('[gSubRoutineCall] parsing error after |') 
        else:
            [node.add(item) for item in cache]
            return node
    ...
    def gUnaryOp(self):
        ...
        if tknode.token in {'-', '~'}:
            ...
            self.ctid+=1
            return node
        raise SyntaxError('gUnaryOp:{}'.format(tknode.token))
    ...
    def gKeyWordConstant(self):
        ...
        if tknode.token in {'true', 'false', 'null', 'this'}:
            ...
            self.ctid+=1
            return node
        raise SyntaxError('gKeyWordConstant:{}'.format(tknode.token))
